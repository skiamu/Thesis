...
% rng default
%% 1) download data
% if the selected freqency is daily or weekly, set freqIn = 'd' or 'wk'.
% Otherwise set freqIn equal of the desired number of week (e.g. 12 for quarterly)
freqIn = 'wk'; % time-series frequency 
if strcmp(freqIn,'d') || strcmp(freqIn,'wk')
	Returns = getTimeSeries(freqIn); % get time-series from Yahoo
else
	
	freqOut = 4; % return frequency (quarterly) used in the model
	Returns = getTimeSeries('wk',freqOut); % get time-series from Yahoo
